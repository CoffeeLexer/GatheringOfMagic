...
router.get(/\/\d+\/duels\/\d+\/decks\/\d+/, async (req, res) => {
    ...
    if(duels.length <= duel_id) return res.status(400).send(`Duel id out of range 0 <= id <= ${duels.length}`)
    let decks = duels[duel_id].decks
    if(decks.length <= id) return res.status(400).send(`Deck id out of range 0 <= id <= ${decks.length}`)
